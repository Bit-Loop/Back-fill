...
from scraper.service.models import BackfillRequest, StreamRequest

# Import new routes structure
from api.rest.routes import registry_router
from api.rest.routes.registry import configure_registry_service
from core.services import RegistryService
from storage.timescale.adapter import TimescaleAdapter
...
async def startup_event() -> None:
    ...
    # Configure new registry service with dependency injection
    config = BackfillConfig.default()
    db = config.database
    ...
    registry_service = RegistryService(repository=adapter)
    configure_registry_service(registry_service)
    logger.info("Configured new registry routes with service layer")
